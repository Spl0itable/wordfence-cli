...
class HumanReadableWriter(ReportWriter):
    ...
    def write_row(self, data: List[str]) -> None:
        ...
        file = values.get(ReportColumn.FILENAME.value)  # Get the filename from the values dictionary
        signature_id = values.get(ReportColumn.SIGNATURE_ID.value)
        ...
        if file is not None:
            if signature_id is not None:
                self._target.write(f"File at {file} matched signature {signature_id}")
            else:
                self._target.write(f"File {file} matched a signature")
        else:
            self._target.write("Match found: " + str(values))
